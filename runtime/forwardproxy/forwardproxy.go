...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request, next caddyhttp.Handler) error {
	...
	if r.Method == http.MethodConnect {
		...
		defer func() {
			if releaseOnFailure {
				releaseV2naiveUser(userID, clientIP)
			}
		}()
		var flushErr error
		...
		case 1: // http1: hijack the whole flow
			return serveHijack(w, targetConn, stats, userLimiter)
		case 2: // http2: keep reading from "request" and writing into same response
			fallthrough
		case 3:
			defer r.Body.Close()
			return dualStream(targetConn, r.Body, w, r.Header.Get("Padding") != "", stats, userLimiter)
		}
...
func serveHijack(w http.ResponseWriter, targetConn net.Conn, stats *streamStats, userLimiter *dynamicRateLimiter) error {
	w.WriteHeader(http.StatusOK)
	...
	return dualStream(targetConn, clientConn, clientConn, false, stats, userLimiter)
}
...
func dualStream(target net.Conn, clientReader io.ReadCloser, clientWriter io.Writer, padding bool, stats *streamStats, userLimiter *dynamicRateLimiter) error {
	stream := func(w io.Writer, r io.Reader, paddingType int, resultCh chan<- streamResult) {
		...
		resultCh <- streamResult{count: n, err: _err}
	}
	...
	if download.err != nil && download.err != io.EOF {
		return download.err
	}
	return nil
}
...
func flushingIoCopy(dst io.Writer, src io.Reader, buf []byte, paddingType int, userLimiter *dynamicRateLimiter) (written int64, err error) {
	...
	for {
		...
		if nr > 0 {
			if err = userLimiter.WaitN(logicalNr); err != nil {
				break
			}
